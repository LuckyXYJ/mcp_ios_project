...
def extract_python_file_info(filepath):
...
def extract_all_code_info(root_path):
    result = {}
    for dirpath, _, filenames in os.walk(root_path):
        for f in filenames:
            if f.endswith(".py"):
                full_path = os.path.join(dirpath, f)
                summary = extract_python_file_info(full_path)
                if summary:
                    result[os.path.relpath(full_path, root_path)] = summary
    return result

@mcp.tool()
def generate_project_summary(root_path: str) -> dict:
    """
    ...
    for file_path, summary in result.items():
        md_filename = os.path.basename(file_path).replace(".", "_") + ".md"
    ...
    return {
        "content": [
            {
                "type": "text",
                "text": f"✅ Project summary generated under `{output_path}`"
            }
        ]
    }
